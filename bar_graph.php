...
// Query to get the count of events by event_type
$sqlEventCounts = "SELECT event_type, COUNT(*) as event_count FROM saved_attendance GROUP BY event_type";
$resultEventCounts = $conn->query($sqlEventCounts);
...
// Fetch event types and their counts
if ($resultEventCounts->num_rows > 0) {
    while ($row = $resultEventCounts->fetch_assoc()) {
        $xValues[] = $row['event_type'];  // Event Type
        $yValues[] = $row['event_count']; // Event Count
        // Generate random color for each bar
        $colors[] = '#' . dechex(rand(0x000000, 0xFFFFFF)); // Random hex color
    }
}

// Query to get counts for age groups
$sqlAgeGroups = "
    SELECT 
        SUM(CASE WHEN age >= 60 THEN 1 ELSE 0 END) AS seniors,
        SUM(CASE WHEN age BETWEEN 13 AND 19 THEN 1 ELSE 0 END) AS teenagers,
        SUM(CASE WHEN age BETWEEN 20 AND 59 THEN 1 ELSE 0 END) AS adults
    FROM residents";
$resultAgeGroups = $conn->query($sqlAgeGroups);

// Variables to store counts
$seniors = $teenagers = $adults = 0;
if ($resultAgeGroups->num_rows > 0) {
    $row = $resultAgeGroups->fetch_assoc();
    $seniors = (int)$row['seniors'];
    $teenagers = (int)$row['teenagers'];
    $adults = (int)$row['adults'];
}

// Fetch the latest saved admin name
$sqlAdminName = "SELECT name FROM admin_names ORDER BY id DESC LIMIT 1";
$resultAdminName = $conn->query($sqlAdminName);
$adminName = "Unknown"; // Default value
if ($resultAdminName->num_rows > 0) {
    $row = $resultAdminName->fetch_assoc();
    $adminName = $row['name'];
}
...
<body>
    ...
    <main>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 -960 960 960" width="48px" fill="#FFFFFF"><path d="M690-160q-12.75 0-21.37-8.63Q660-177.25 660-190v-220q0-12.75 8.63-21.38Q677.25-440 690-440h80q12.75 0 21.38 8.62Q800-422.75 800-410v220q0 12.75-8.62 21.37Q782.75-160 770-160h-80Zm-250 0q-12.75 0-21.37-8.63Q410-177.25 410-190v-580q0-12.75 8.63-21.38Q427.25-800 440-800h80q12.75 0 21.38 8.62Q550-782.75 550-770v580q0 12.75-8.62 21.37Q532.75-160 520-160h-80Zm-250 0q-12.75 0-21.37-8.63Q160-177.25 160-190v-380q0-12.75 8.63-21.38Q177.25-600 190-600h80q12.75 0 21.38 8.62Q300-582.75 300-570v380q0 12.75-8.62 21.37Q282.75-160 270-160h-80Z"/></svg>
            ...
            <div class="hgrid">
                <!-- Print Button -->
                <button onclick="printGraph()" style="margin: 20px; padding: 10px 20px; background-color: #4CAF50; color: white; border: none; cursor: pointer;">
    Download Statistics as Word Document
</button>
   ...
   // Function to download the graph and records as a Word document
function printGraph() {
    ...
    try {
        ...
        // Prepare event records data
        const records = xValues.map((eventType, index) => ({
            eventType: eventType,
            count: yValues[index]
        }));

        // Enhanced Word document template with data moved upward
        const content = `
            <html xmlns:w="urn:schemas-microsoft-com:office:word">
                <head>
                    <meta charset="UTF-8">
                    <style>
                        body { font-family: 'Arial', sans-serif; margin: 5px; padding-top: 0; } /* Reduced top margin */
                        .container { max-width: 800px; margin: 0 auto; }
                        .header { text-align: center; margin-bottom: 15px; } /* Reduced margin */
                        .header img { width: 150px !important; height: 150px !important; margin-bottom: 5px; } /* Reduced margin */
                        h1 { font-size: 24px; margin: 5px 0; color: #2E2E2E; } /* Reduced margin */
                        h2 { font-size: 20px; color: #444; }
                        h3 { font-size: 18px; margin: 10px 0 5px 0; } /* Reduced top margin */
                        table { width: 100%; border-collapse: collapse; margin: 10px 0; } /* Reduced margin */
                        th, td { border: 1px solid #333; padding: 8px; text-align: left; } /* Slightly reduced padding */
                        th { background-color: #f2f2f2; font-weight: bold; }
                        img.chart { max-width: 100%; height: auto; display: block; margin: 10px auto; } /* Reduced margin */
                        .footer { text-align: right; font-style: italic; font-size: 14px; margin-top: 15px; } /* Reduced margin */
                        .date { font-size: 12px; color: #666; margin-top: 2px; } /* Reduced margin */
                    </style>
                </head>
                <body>
                    <div class="container">
                        <!-- DOCUMENT START -->
                        <div class="header">
                            <img src="http://localhost/dalla/img/logo.jpg" alt="BDAMS Logo" width="130" height="130">
                            <h1>Barangay Dalla Attendance Monitoring System</h1>
                            <div class="date">Generated on: ${new Date().toLocaleString()}</div>
                        </div>

                        <h3>Event Records Summary</h3> <h3>Event Records Summary</h3>
                        <table>
                            <thead>
                                <tr>
                                    <th>Event Type</th>
                        ...
                        <div class="footer">
                            Printed by: ${adminName || 'Unknown Administrator'}
                        </div>
                        <!-- DOCUMENT END -->
                    </div>
                </body>
        ...
        link.download = `Event_Statistics_${new Date().toISOString().slice(0,10)}.doc`;
        ...
        alert("Failed to generate the document. Please check the console for details.");
    }
}
</script>
    ...
    <script>
   function sendPrintRequest(page, type) {
    var printWindow = window.open(page + '?print=' + type, '_blank');

    printWindow.addEventListener('load', function () {
        setTimeout(function () { 
            if (printWindow) {
                if (type === 'all' && typeof printWindow.printAllSummary === 'function') {
                    printWindow.printAllSummary();
                } else if (type === 'daily' && typeof printWindow.printDailySummary === 'function') {
                    printWindow.printDailySummary();
                } else if (type === 'monthly' && typeof printWindow.printMonthlySummary === 'function') {
                    printWindow.printMonthlySummary();
                } else if (type === 'yearly' && typeof printWindow.printYearlySummary === 'function') {
                    printWindow.printYearlySummary();
                }
